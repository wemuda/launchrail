...
- **The workflow plugin** carries the `launch` and `start-feature` conductors ([ADR-0009](docs/adr/0009-launch-orchestrator-skill.md), [ADR-0014](docs/adr/0014-start-feature-conductor.md)), the `vision-creation`, `discovery`, `design-validation`, and `project-alignment` skills, and a stage-by-stage workflow doc that composes Matt Pocock's grill/research/spec skills. The `discovery` stage is the divergent option-space scan before the grill — it maps the real vendors/libraries/patterns for the vision's hard parts, then hands the landscape to the grill ([ADR-0015](docs/adr/0015-discovery-research-stage.md)). Subscription is a project-scoped `.claude/settings.json` declaration ([ADR-0003](docs/adr/0003-plugin-subscription-via-project-settings.md)).
...
- **Regenerate the `how-launchrail-works` diagram.** `assets/how-launchrail-works.png` and its README alt text predate the `discovery` stage ([ADR-0015](docs/adr/0015-discovery-research-stage.md)); redraw the foundation as Vision → Visual exploration → Discovery → Grill → Technical research → ADRs so the front-door diagram matches the contract.
