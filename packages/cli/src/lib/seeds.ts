...
- In a hosted session (a fresh clone) run \`node scripts/setup.mjs\` first, unconditionally. Then start the stack with \`npx @wemuda/launchrail dev --background\` (\`--port <n>\` when other builders share the machine): it runs \`smoke.start\` from \`.launchrail.yml\`, else \`testing.devCommand\`, waits until every origin answers, and writes \`.launchrail/state/stack.json\` (origins, pid, log — plus whatever a composed start command adds: keys, fixtures, a mailbox) and \`browser.env\`. Stop it with \`launchrail dev --stop\`; prove the contract with \`launchrail dev --check\`.
- The smoke's driver is \`agent-browser\` (installed by \`node scripts/setup.mjs\`), driven from the shell — \`open\`, \`snapshot -i\`, \`click @ref\`, \`fill\`, \`wait\`, \`screenshot\`, \`errors\`, \`console\`, \`network requests\` — one \`--session\` per ticket. Source \`.launchrail/state/browser.env\` first so it reuses Playwright's Chromium.
- A stack that \`dev\` cannot start from the manifest is a blocker for the smoke, not something to rebuild by hand inside a ticket: the fix is a \`smoke.start\` command (see the skill's \`composed-stack.md\`).
