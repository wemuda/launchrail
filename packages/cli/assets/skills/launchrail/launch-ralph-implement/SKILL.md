...
7. **Commit to the current branch** following the project's commit conventions (Conventional Commits when `.launchrail.yml` says so); the loop merges your branch onto the base — your commits are preserved, not squashed — so also hand it a Conventional Commit title for the merge commit. Update any artifact the change invalidates (spec, ADR, design note) in the same change.
