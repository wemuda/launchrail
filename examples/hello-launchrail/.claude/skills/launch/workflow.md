...
- The repository is initialized (`npx @wemuda/launchrail init`) and healthy (`npx @wemuda/launchrail doctor`). Init writes the workflow skills *and* the default implementation loop's materials — there is no separate install step on the golden path.
...
| 10 | Implementation | `/launch-implement` † → the selected loop (`implementationLoop`, default Ralph) | Ready tickets | PRs merged and verified; the frontier drained |
...
† **User-typed by design** — `disable-model-invocation`: only the user can start these (upstream's choice for `/setup-matt-pocock-skills` and `wayfinder`/`to-spec`; Launchrail's own for `/launch-implement`, because implementation spawns agents and merges PRs). A conductor prepares the handoff instead of calling them — see the conductor rules.
...
- **Stage 4 is `grill-with-docs`, never the bare `grilling` primitive.** Both are vendored with the workflow, but only `grill-with-docs` writes the `docs/research/` artifact the stage gates on — the primitive produces conversation and no committed file.
...
- **Stage 10 is provider-selected behind one door.** `/launch-implement` reads `implementationLoop` (`ralph` default, `superpowers` selectable — [ADR-0017](https://github.com/wemuda/launchrail/blob/master/docs/adr/0017-implementation-loop-provider.md)) and routes to that engine. Launchrail owns both edges whichever loop runs: `ready-for-agent` tickets with `Blocked by: #n` edges in, `launchrail verify` (+ browser smoke where enabled) gating every merge.
