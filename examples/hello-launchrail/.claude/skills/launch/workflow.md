...
† **User-typed by design** — `disable-model-invocation`: only the user can start these. `launch-wayfinder`/`launch-spec` and `launch-tickets` publish to the tracker; `/launch-implement` spawns agents and merges PRs. `/launch-design-implement` — not a stage, but the faithful build of a design package (see *Returning from Claude Design*) — is user-typed for the same reason: implementation is never started unprompted. A conductor prepares the handoff instead of calling them — see the conductor rules.
...
When the prototype *is* the spec — a redesign of an existing page, or a new page whose behavior the canvas fully shows — the handoff proposes a **faithful build** instead of a size: the user-typed `/launch-design-implement "<canvas>"` — one invocation, taking the dropped export directly and composing the on-ramp to package it on the way, or a package already committed — builds that one canvas under the [fidelity protocol](../launch-design-implement/fidelity.md) ([design-fidelity-build](https://github.com/wemuda/launchrail/blob/master/docs/adr/2026-09-17-design-fidelity-build.md)). The state inventory and the token, component, and data maps are written to `docs/design/<slug>/fidelity.md` before the build; the product is built from the prototype's own markup and styles, state by state; and every state is compared side by side with the running prototype at the designed viewport, its verdict recorded, until each row is a match or an accepted deviation with a reason. A ticket that names a design package follows the same protocol inside the loop through `launch-ralph-implement`, and the browser smoke's named design comparison is the protocol's side-by-side — one definition of "true to the design", wherever a design gets built.
