...
A feature that arrives **design-first** — a dropped zip or folder of Claude Design artboards, "here is the prototype of X" — routes through `launch-design-handoff` before sizing: it commits the package under `docs/design/<slug>/` and proposes a size; sizing then consumes its `handoff.md` as the feature brief, the grill takes the doc's open questions as its agenda, the spec cites the package as its UX/UI reference, and design validation usually becomes a recorded skip citing it. When the prototype *is* the spec — a redesign of an existing page, or a new page the canvas fully shows — the handoff proposes the **faithful build** instead of a size: `/launch-design-implement "<canvas>"`, user-typed like `/launch-implement`, builds that one canvas from the prototype's own code and verifies it side by side at the designed viewport ([fidelity protocol](../launch-design-implement/fidelity.md)) — hand the command over, never start it. It is one invocation: a user who drops an export and asks for it built types that command directly, and it packages the drop through the on-ramp on the way.
...
- `setup` / `init` — 0 · `readiness` / `tune` / `optimize` — `launch-loop-readiness` · `align` / `adopt` — the existing-project on-ramp · `vision` — 1 · `explore` — 2 · `discovery` / `landscape` — 3 · `grill` — 4 · `research` — 5 · `deep-research` — 3→5 · `adr` / `architecture` — 6 · `spec` — 7 · `design-validation` / `validate` — 8 · `tickets` — 9 · `implement` / `build` / `ralph` / `loop` — hand over `/launch-implement` · `verify` / `smoke` — 11 · `release` — 12 · `handoff` / `design-handoff` — the design→code on-ramp (`launch-design-handoff`) · `design-implement` / `build design` / `implement design` — hand over `/launch-design-implement` (user-typed; one invocation — it takes a dropped export directly and packages it through the on-ramp on the way, or a package already committed under `docs/design/<slug>/`).
