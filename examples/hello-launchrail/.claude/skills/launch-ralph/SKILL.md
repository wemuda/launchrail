...
4. **Know the healthy shapes so you don't cry wolf.** A ticket can appear twice in Build — that is the retry policy, or a *deferral* because its dependency had not landed yet (not a failure). Build ending at PR-open with a separate Gate agent doing the merge is the design, not a stall. Several Gate dispatches for one PR (`gate:#n`, then `gate:#n:ci-wait1…`, then `gate:#n:ci-done…`) are the loop waiting out a still-running CI — the `ci-wait` turns are read-only small-model watchers, and the real gate returns once CI resolves — cheap, expected on anything but the fastest CI, and *not* a build attempt: a slow CI costs cheap watches, never a rebuild and never a stack of full-model gates (ADR-0027, ADR-0030). A ticket only truly fails after two real attempts, then it parks.
