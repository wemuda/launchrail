...
- **`launch-design-implement`** — builds one canvas of a committed handoff package *true to the prototype*: the state inventory and the token, component, and data maps are written first, the product is built from the prototype's own markup and styles state by state, and every state is compared side by side with the running prototype at the designed viewport until each row matches or is an accepted deviation with a reason ([design-fidelity-build](docs/adr/2026-09-17-design-fidelity-build.md)); user-typed, like `launch-implement`
