...
- **`launch-design-implement`** — one invocation from a Claude Design export to a built page, *true to the prototype*: drop the zip, name the canvas, and it packages the drop (through the on-ramp), writes the state inventory and the token, component, and data maps first, the product is built from the prototype's own markup and styles state by state, and every state is compared side by side with the running prototype at the designed viewport until each row matches or is an accepted deviation with a reason ([design-fidelity-build](docs/adr/2026-09-17-design-fidelity-build.md)); user-typed, like `launch-implement`
...
- **`launch-browser-smoke`** — drives the running app in a real browser to see a just-built change working — one-off, from the shell with `agent-browser` against a stack `launchrail dev` started, no test suite and no evidence bundle ([ADR-0034](docs/adr/0034-browser-smoke-one-off-driving.md), [ADR-0036](docs/adr/0036-cli-owned-stack-start.md); with the browser-testing module)
...
Launchrail's skill set is its own, but much of its methodology traces to [**Matt Pocock**](https://www.mattpocock.com/). The grill, research, wayfinding, spec, ticket, and code-review stages absorb the shape — and, in places, the MIT-licensed text — of his [`skills`](https://github.com/mattpocock/skills) repository ([ADR-0020](docs/adr/0020-independent-skill-set.md)); the derived skills carry the attribution in `NOTICE.md`, and upstream is monitored so improvements there keep informing the rail.
