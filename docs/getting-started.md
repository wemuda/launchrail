...
5. **Coming back from Claude Design with a zip?** Drop it into the session: `launch-design-handoff` commits it as a handoff package under `docs/design/` and proposes a path. When the prototype *is* the spec — a redesign of a page, or a new page the canvas fully shows — `/launch-design-implement <slug> "<canvas>"` builds that canvas true to it: from the prototype's own code, state by state, verified side by side in a real browser at the size you approved it at.
