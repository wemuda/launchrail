...
- [Claude Code](https://claude.com/claude-code) if you want to run the workflow skills (the CLI works without it, and the skills are plain files any repo-reading agent can use)
...
The interview asks three things — whether this is a new project or an existing one you're adopting, your issue tracker, and whether to use Conventional Commits — with defaults detected from the repo (package manager, scripts, commit history). It doesn't ask for a test command: init records the repo's test script as `testing.unitCommand` when one exists and otherwise leaves it null in the manifest, for the workflow (or you) to fill in once the stack exists. It then writes:
...
The interview's first question — **new or existing project** — is recorded as `origin` in `.launchrail.yml`; it defaults to `existing` when it detects a `package.json` or existing agent files. That answer changes how the workflow starts: for an existing project, `/launch` takes the **alignment on-ramp** ([ADR-0013](adr/0013-existing-project-alignment.md)) instead of a blank vision. The `project-alignment` skill inventories what your codebase already has, infers a draft vision from the code, interviews you only about the gaps (the real target user, the bet, the success signal), and detects your existing design system as the baseline for later design stages — then hands to `vision-creation` to commit and routes you to the first real gap. Everything after the vision is the same loop; alignment just gets an adopted project onto the rail without re-asking what the code already answers.
...
`doctor` checks the manifest, lockfile, checksum drift, pending migrations, the skills, and environment, and exits non-zero on failures (warnings don't fail it).
...
5. **Coming back from Claude Design with a zip?** Drop it into the session and type `/launch-design-implement "<canvas>"` — one invocation. It commits the export as a handoff package under `docs/design/` on the way, then builds the named canvas true to the prototype: from its own code, state by state, verified side by side in a real browser at the size you approved it at. (Want the design sized onto the rail as a feature instead — grill, spec, tickets? `launch-design-handoff` alone files it and proposes a path.)
