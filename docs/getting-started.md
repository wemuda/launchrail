...
With the skills in `.claude/skills/`, the development loop runs vision → discovery → grill → research → ADRs → spec → design validation → tickets → bounded implementation → verification → release. The stage contract lives in the [workflow doc](../packages/cli/assets/skills/launchrail/launch/workflow.md) (written into projects as `.claude/skills/launch/workflow.md`).
