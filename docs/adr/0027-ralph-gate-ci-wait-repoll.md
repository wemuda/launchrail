...
## Status
Superseded by [ADR-0032](0032-ralph-lean-local-gate-loop.md) — the per-ticket cloud-CI wait no longer exists (the loop lands locally under a fast gate), so there is nothing to re-poll. What survives: "a not-done-yet gate must never cost a rebuild" lives on as the land hand-back that re-syncs a pushed branch without spending an attempt. Was: Accepted (amends [ADR-0022](0022-ralph-campaign-revision.md)) — amended by [ADR-0030](0030-ralph-ci-wait-cheap-watcher.md): the in-place re-polls now ride on cheap read-only small-model watchers, with the full gate recalled only once CI resolves
