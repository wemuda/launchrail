...
## Status
Accepted — amended by [ADR-0023](0023-remove-project-mode.md): the manifest `mode` is retired, so the level recommendation reads the spec's design surface alone; the recommend-then-confirm contract and the ladder itself are unchanged.
