...
- **The module seeds an optional Playwright MCP (`.mcp.json`).** The `browser-smoke` skill drives journeys through "Playwright MCP … whichever is available," so the module makes one available out of the box: a seeded-class, app-agnostic (`--isolated`, no hardcoded origins) `.mcp.json`. Seeded-class means an existing `.mcp.json` is never overwritten, so a project's other MCP servers are safe. This does **not** make the MCP a hard dependency (see Alternatives) — where no MCP is present the seeded Playwright scripts still run headless, which is what keeps cloud/CI sessions working.
...
- **Playwright MCP as the mandated agent interface** — rejected as a hard dependency; availability varies by environment. The skill works with whatever browser tooling the session has, and an adapter abstraction can come later if a non-Playwright layer is ever needed. The module now *seeds* an optional Playwright MCP config (above) so the capability is available where the environment supports it, without mandating it — the deterministic scripts remain the floor.
