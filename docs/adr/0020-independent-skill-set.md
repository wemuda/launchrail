...
1. **The workflow-critical upstream skills are absorbed, not composed.** Each becomes a Launchrail skill written to the rail's contract (committed artifacts, stage gates, the `verify` gate, the rail's label vocabulary):
   ...
   - `grilling` + `grill-with-docs` → `launch-grill` (one skill; the interview always ends in the committed `docs/research/` constraints doc, eliminating the "never bare `grilling`" footgun; maintains the domain model via `launch-domain-modeling`)
   - `domain-modeling` → `launch-domain-modeling` (glossary discipline kept; ADRs use the project's seeded `docs/adr/0000-template.md`, not a second format)
   ...
   - `setup-matt-pocock-skills` → `launch-setup` (issue-tracker and domain-doc configuration under `docs/agents/`, same output location; the triage-label section is dropped with the `triage` skill and the rail's fixed labels are recorded in the tracker doc)
