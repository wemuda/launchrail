...
| [0005](0005-ralph-two-frontends-one-policy.md) | Ralph: two frontends, one policy, verification-gated | Accepted — amended by 0010, 0018; extended by 0021 |
...
| [0010](0010-ralph-field-revision.md) | Ralph field revision (deterministic edges, deferrals, supervision) | Accepted — amends 0005 |
...
- **The implementation loop — read 0005, 0010, 0018, 0021.** Ralph is *the* loop (the provider seam of 0017 was removed by 0020). Two frontends (skill and workflow) share one policy; completion is gated on `launchrail verify`; the user-typed hard gate lives on `/launch-implement`; the permission guard warns on unattended launches in interactive modes.
