...
1. **Confirm the on-ramp applies.** Read `.launchrail.yml`. This skill is for `origin: existing`; if the manifest says `origin: new`, say so and route to `launchrail:vision-creation` instead — a blank start doesn't need alignment.
