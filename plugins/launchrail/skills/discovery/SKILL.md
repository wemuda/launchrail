...
- **Compose, never duplicate.** Discovery is Launchrail-owned framing over Matt Pocock's research skill. Do the divergent framing yourself — carve the product into areas, enumerate contenders — and invoke the research skill to go deep on any single thread that needs primary sources (real capabilities, maintenance health, license, integration cost). Don't reimplement research; drive it.
...
- **Evidence over vibes.** A contender listed from memory is a lead, not a finding. Where a choice is load-bearing, confirm the fact — does it actually do X, is it maintained, what's the license — through the research skill rather than asserting it.
...
4. **Go deep where it's load-bearing.** For the areas where the choice most shapes the architecture, drive Matt Pocock's research skill on the specific threads — verify real capabilities against the vision's needs, maintenance and community health, license, and concrete integration cost on this stack. Where the research skill isn't available in the session, say so and fall back to a clearly-marked best-effort survey — do not silently skip the depth pass.
...
6. **Hand to the grill.** This stage does not choose. Route to the complexity grill (`grill-with-docs`, stage 4), which takes the landscape as input and narrows it into surviving constraints. Tell the user what you surveyed, where the docs are, that the grill is next, and that they can jump straight there.
