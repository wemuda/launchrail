...
description: Implement a single ticket end to end under the Launchrail completion contract — TDD, the deterministic verification gate, browser smoke for user-facing changes, self-review, and conventional commits. Used by Ralph campaign dispatches and equally valid for implementing one ticket by hand.
---
...
4. **User-facing behavior, with `modules.browser-testing` enabled:** update or add the affected journey in `docs/testing/smoke-journeys.md` and drive it per the `launchrail:browser-smoke` skill. A journey you could not complete is a failure, not a pass.
5. **Self-review:** run `/code-review` on the result and fix what it finds before handing off.
