...
From `.launchrail.yml`: `implementationLoop` (default `ralph`), `issueTracker`, and the `testing` commands. From the arguments: the scope —
...
**The frontier (no arguments), `ralph`:** run the loop under the `launchrail:ralph` skill — it owns the policies (width, attempts, deferrals, remote-verified merges) and the orchestrator's contract. For a wide dependency graph or a long run, prefer its workflow form (`.claude/workflows/ralph.js`, scope and policy via args) — the skill explains when.
