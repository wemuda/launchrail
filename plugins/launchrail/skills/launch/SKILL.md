...
- **Never start the implementation loop unprompted.** Stage 10 is user-invoked only, whichever loop the project selected. Route the user to it and explain; do not launch it yourself.
...
| 8 | Design validation | `launchrail:design-validation` — scaled to the spec's UI surface (see below) | The spec carries a `## Design validation` section (a recorded skip counts) |
| 9 | Tickets | Matt Pocock `to-tickets` | The tracker has `ready-for-agent` tickets with `Blocked by: #n` edges |
| 10 | Implementation | The project's selected loop (`.launchrail.yml` `implementationLoop`; default `launchrail:ralph` †, needs `launchrail add ralph`) | The ready frontier is drained; PRs merged and verified |
...
**Stage 8 scales to the spec's design surface — judge it, don't turnstile it.** Design validation checks the spec's user-facing flows on screen *before* tickets exist, so a "specified but wrong on screen" finding costs a spec edit rather than re-cut tickets or reworked code — that is why it sits before stage 9, and it is not stage 11, which checks the *built* product. It runs at one of four fidelity levels — recorded skip, flow-diagram artifact, screen-mockup artifact, or Claude Design ([ADR-0016](../../../../docs/adr/0016-design-validation-fidelity-ladder.md)) — and the level choice lives *inside* `launchrail:design-validation` (recommend from surface and `mode`, user confirms), not in your routing. Your job after stage 7 is only to note the spec's surface when routing: UI-heavy specs make the stage worth its cost; an API-, schema-, or infrastructure-heavy spec (a few thin screens or none) gets a recommendation to move on to tickets, with the stage offered at a low level — the same judgment `start-feature` applies when sizing a feature. Even a recorded skip goes through `launchrail:design-validation` to write the `## Design validation` section (stating what was assessed and why there was little or nothing to drive), so the stage-9 gate stays artifact-based instead of becoming chat memory.

**Stage 10 is provider-selected — read `implementationLoop`, then route.** The implementation loop is the engine that drives ready tickets to verified merges, and the project picks it in `.launchrail.yml` (`implementationLoop`; default `ralph`). Launchrail owns *both edges* regardless of which loop runs: tickets enter with the `ready-for-agent` label and `Blocked by: #n` edges, and every merge passes `npx @wemuda/launchrail verify` (+ browser smoke where enabled) — that gate is stage 11 and it is non-negotiable no matter the loop. Route stage 10 to the selected provider: `ralph` → `launchrail:ralph` (needs `launchrail add ralph`); `superpowers` → drive the ready tickets through `superpowers:executing-plans` with `superpowers:test-driven-development`, closing each branch with `superpowers:finishing-a-development-branch` (the Superpowers plugin is installed and declared by `init` when selected). Never pick a loop for the user or start one unprompted ([ADR-0017](../../../../docs/adr/0017-implementation-loop-provider.md)).

**`ready-for-agent` marks tickets, never specs.** The implementation loop's frontier is *every open issue* wearing the `ready-for-agent` label — the loop cannot tell prose from work, so a spec, research note, or epic published to the tracker with that label will be dispatched to an implementer as if it were a ticket. Issues that publish workflow artifacts (a spec, research notes) take a different label (e.g. `spec`); only stage-9 output — implementable tickets carrying `Blocked by: #n` edges — ever takes `ready-for-agent`. If you find a non-ticket wearing it, have it relabeled before anyone starts the loop.
...
4. **Route.** Invoke the owning skill by its exact name (`launchrail:vision-creation`, `launchrail:design-validation`, `launchrail:browser-smoke`), or hand off to the upstream skill / CLI command the map names. For a **user-typed** stage (†) you can't invoke, prepare the handoff instead of calling and getting refused: for stage 7, hand over the exact, fully-argumented `wayfinder` / `to-spec` command with the authoritative inputs named, so it builds on the settled decisions rather than re-exploring the codebase — and if the stack isn't stood up yet, tell it to name its seams but leave the harness mechanism (test database, CI wiring) to the foundation work, rather than pinning infrastructure the stack hasn't chosen; for stage 10, read `implementationLoop`, explain the selected loop, and let the user start it themselves (default `launchrail:ralph`).
5. **Always leave a map — an explained one.** Whatever you route to, tell the user their current stage and the next one, with one sentence on what that next stage actually *does* and whether it is optional for this project — a bare stage name explains nothing and reads as a mandatory turnstile. Then show the rest of the arc to the destination so it's visible where the rail is heading (e.g. after a spec: design validation *if the spec has UI surface* → tickets → the implementation loop → verification), and that they can jump to any stage by keyword.
...
- `tickets` — stage 9.
- `implement` / `ralph` / `loop` — stage 10, the selected implementation loop (hand off for explicit start).
