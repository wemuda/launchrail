...
5. **Always leave a map — an explained one.** Whatever you route to, tell the user their current stage and the next one, with one sentence on what that next stage actually *does* and whether it is optional for this project — a bare stage name explains nothing and reads as a mandatory turnstile. Then show the rest of the arc to the destination so it's visible where the rail is heading (e.g. after a spec: design validation *if the spec has UI surface* → tickets → the Ralph loop → verification), and that they can jump to any stage by keyword.
