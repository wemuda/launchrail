...
4. **Route.** Invoke the owning skill by its exact name (`launchrail:vision-creation`, `launchrail:design-validation`, `launchrail:browser-smoke`), or hand off to the upstream skill / CLI command the map names. For stage 10, read `implementationLoop`, explain the selected loop, and let the user start it themselves (default: `launchrail:ralph`).
