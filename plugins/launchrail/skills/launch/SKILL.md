...
2. **Otherwise, find the frontier.** Confirm setup (stage 0) first — and close its gaps yourself, without asking (see ground rules): if `.launchrail.yml` is missing, run `init`; if the init output is untracked, commit it; if `docs/agents/` is missing, tell the user to type `/setup-matt-pocock-skills` (user-invoked upstream — hand them the manifest-derived answers so it's a ten-second run). Then walk stages 1 → 11 in order and stop at the first whose "done when" is not satisfied. Skip only the stages the manifest's `mode` permits skipping (see below).
