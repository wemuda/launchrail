...
**Stage 8 scales to the spec's design surface — judge it, don't turnstile it.** Design validation drives the spec's user-facing flows through Claude Design *before* tickets exist, so a "specified but wrong on screen" finding costs a spec edit rather than re-cut tickets or reworked code — that is why it sits before stage 9, and it is not stage 11, which checks the *built* product. It only pays where the spec has real UI surface. When routing after stage 7, read the spec's surface first: a UI-heavy spec gets the full validation pass; an API-, schema-, or infrastructure-heavy spec (a few thin screens or none) gets a recommendation to move on to tickets, with validation offered as a quick pass over the screens that do exist or as a recorded skip — the same judgment `start-feature` applies when sizing a feature. A recorded skip still goes through `launchrail:design-validation` to write the `## Design validation` section (stating what was assessed and why there was little or nothing to drive), so the stage-9 gate stays artifact-based instead of becoming chat memory.
