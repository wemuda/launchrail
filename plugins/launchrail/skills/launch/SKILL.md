...
| 7 | Design validation | `launchrail:design-validation` — scaled to the spec's UI surface (see below) | The spec carries a `## Design validation` section (a recorded skip counts) |
...
**Stage 7 scales to the spec's design surface — judge it, don't turnstile it.** Design validation drives the spec's user-facing flows through Claude Design *before* tickets exist, so a "specified but wrong on screen" finding costs a spec edit rather than re-cut tickets or reworked code — that is why it sits before stage 8, and it is not stage 10, which checks the *built* product. It only pays where the spec has real UI surface. When routing after stage 6, read the spec's surface first: a UI-heavy spec gets the full validation pass; an API-, schema-, or infrastructure-heavy spec (a few thin screens or none) gets a recommendation to move on to tickets, with validation offered as a quick pass over the screens that do exist or as a recorded skip — the same judgment `start-feature` applies when sizing a feature. A recorded skip still goes through `launchrail:design-validation` to write the `## Design validation` section (stating what was assessed and why there was little or nothing to drive), so the stage-8 gate stays artifact-based instead of becoming chat memory.

**`ready-for-agent` marks tickets, never specs.** The Ralph loop's frontier is *every open issue* wearing the `ready-for-agent` label — the loop cannot tell prose from work, so a spec, research note, or epic published to the tracker with that label will be dispatched to an implementer as if it were a ticket. Issues that publish workflow artifacts (a spec, research notes) take a different label (e.g. `spec`); only stage-8 output — implementable tickets carrying `Blocked by: #n` edges — ever takes `ready-for-agent`. If you find a non-ticket wearing it, have it relabeled before anyone starts Ralph.
