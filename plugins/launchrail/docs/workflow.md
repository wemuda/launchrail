...
Stage 8 scales to the spec's design surface. Design validation catches "specified but wrong on screen" while a finding still costs a spec edit rather than re-cut tickets or reworked code — that is why it sits before tickets, and why it is not the post-implementation check (that is browser smoke / `verify`, on the built product). A UI-heavy spec gets the full pass; an API-, schema-, or infrastructure-heavy spec gets a quick pass over the few screens it has, or a recorded skip. Either way the `## Design validation` section gets written — it is the stage's artifact gate, and a recorded skip states what was assessed and why there was nothing to drive.
